...
parser.add_argument('--img_root', type=str, default='./dataset', help='path name of image dataset')
...
parser.add_argument('--data_name', type=str, required=True,default='cifar10', help='name of dataset') # cifar10/cifar100
...
parser.add_argument('--lambda_kd', type=float, default=50, help='trade-off parameter for kd loss')
...
def main():
	...
	elif args.kd_mode =='STandNST':
		criterionKD = STandNST(args.T)
	...
	train_loader = torch.utils.data.DataLoader(
			dataset(root      = args.img_root,
					...
					train     = True,
					download  = True),
			batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)#4
	test_loader = torch.utils.data.DataLoader(
			dataset(root      = args.img_root,
					transform = test_transform,
					train     = False,
					download  = True),
			batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)#4
	...
	for epoch in range(1, args.epochs+1):
		...
		test_top1, test_top5 = test(test_loader, nets, criterions, epoch,clsloss_list,top1_list,top5_list,kdloss_list)
...
def train(train_loader, nets, optimizer, criterions, epoch):
	...
	for i, (img, target) in enumerate(train_loader, start=1):
		...
		elif args.kd_mode in ['fitnet', 'nst','nstnew']:
			kd_loss = criterionKD(rb3_s[1], rb3_t[1].detach()) * args.lambda_kd
		elif args.kd_mode in ['STandNST']:
			kd_loss = criterionKD(rb3_s[1], rb3_t[1].detach(),0) * 50+criterionKD(out_s, out_t.detach(),1)
...
def test(test_loader, nets, criterions, epoch,clsloss_list,top1_list,top5_list,kdloss_list):
	cls_losses = AverageMeter()
	kd_losses  = AverageMeter()
	top1       = AverageMeter()
	top5       = AverageMeter()
	...
	clsloss_list.append(cls_losses.avg)
	kdloss_list.append(kd_losses.avg)
	top1_list.append(top1.avg)
	top5_list.append(top5.avg)
